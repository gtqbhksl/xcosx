...
func NewArtifact(img types.Image, c cache.ArtifactCache, opt artifact.Option) (artifact.Artifact, error) {
	...
	// Register Webshell analyzers
	if err := webshell.RegisterWebshellAnalyzer(opt.WebshellScannerOption); err != nil {
		return nil, xerrors.Errorf("file analyzer error: %w", err)
	}
...
func (a Artifact) calcCacheKeys(imageID string, diffIDs []string) (string, []string, map[string]string, error) {
	...
	imageKey, err := cache.CalcKey(imageID, a.analyzer.ImageConfigAnalyzerVersions(), artifact.Option{})
	if err != nil {
		return "", nil, nil, err
	}
	...
	for _, diffID := range diffIDs {
		blobKey, err := cache.CalcKey(diffID, a.analyzer.AnalyzerVersions(), a.artifactOption)
		if err != nil {
			return "", nil, nil, err
		}
...
func (a Artifact) inspectLayer(ctx context.Context, diffID string, disabled []analyzer.Type) (types.BlobInfo, error) {
	...
	blobInfo := types.BlobInfo{
		...
		Weakpasses:      result.Weakpasses,
		WebshellResult:  result.WebshellResult,
		OpaqueDirs:      opqDirs,
