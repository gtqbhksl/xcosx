...
func ApplyLayers(layers []types.BlobInfo) types.ArtifactDetail {
	...
	for _, layer := range layers {
		...
		for _, weakpass := range layer.Weakpasses {
			key := fmt.Sprintf("%s/type:%s u%s p%s", weakpass.FilePath, weakpass.Type, weakpass.Username, weakpass.Password)
			nestedMap.SetByString(key, sep, weakpass)
		}

		for _, webshell := range layer.WebshellResult {
			if len(webshell.Source) > 0 {
				key := fmt.Sprintf("%s/type:%s s%s", webshell.FilePath, webshell.Source, webshell.Source)
				nestedMap.SetByString(key, sep, webshell)
			}
	...
	_ = nestedMap.Walk(func(keys []string, value interface{}) error {
		...
		case types.Weakpass:
			mergedLayer.Weakpasses = append(mergedLayer.Weakpasses, v)
		...
		case types.WebshellResult:
			mergedLayer.WebshellResult = append(mergedLayer.WebshellResult, v)
		}
